  ...
  if(max(evi) > 0.1){
    ...
    tryCatch({ 
      ...
      if(evi_ratio > 1.5){
        ...
        minima_indices <- which(diff(sign(diff(third_derivative))) == 2)  # min
        maxima_indices <- which(diff(sign(diff(third_derivative))) == -2)  # max
...
extract_leafoff <- function(NTL_matrix, i, year = year, span = 0.8, MOFs, EOFs) {
  ...
  if (length(valid_idx) >= floor(0.4*length(season_mask)) | length(valid_idx) >= 15) {
    ...
    if (!is.null(logistic_model) && is_s_curve_by_second_derivative(original_scale_pred, t_full) &&
        rcorr(cbind(loess_fit, original_scale_pred), type = "pearson")$r[1,2] > 0.5) {
      
      if (!(all(is.na(logistic_pred[1:10])) | all(is.na(logistic_pred[(length(season_mask)-10):length(season_mask)])))) {
        date <- which.max(diff(c(original_scale_pred[1],original_scale_pred)))
